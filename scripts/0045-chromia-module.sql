...
-- truth and has drifted from schema.prisma in known-safe ways (FK renames,
-- updated_at DROP DEFAULT on Sales tables, 3 index tweaks). A push would try to
-- "fix" that drift. Every statement below names only chromia_* objects, plus
-- the one additive ALTER TYPE on Role — zero DROPs, zero statements touching an
-- existing table.
--
-- User attribution columns (*_by_id, user_id, operator_id, inspector_id) are
-- plain TEXT holding users.id with NO foreign key — the Sales precedent — so no
-- FK ties Chromia rows to the User table and a slab's history survives a user
-- being archived. chromia_user is the module's dormant employee directory, kept
...
--
-- NOTE ON THE ENUM AND TRANSACTIONS: PostgreSQL runs a multi-statement script as
...
-- Role enum has never been given its CHROMIA value, i.e. this script has not run
-- before. Once it has, re-running stays a no-op, as the header promises.
DO $$ DECLARE n bigint; BEGIN
  IF to_regclass('chromia_slab') IS NOT NULL
