...
import { PrintButton } from "../ceo/PrintButton";
import { WidthToggle } from "../ceo/WidthToggle";
import s from "../ceo/report.module.css";
 ...
 * so the two reports describe the same day in the same words. */
...
/* ---------------------------------------------------------------- page */
export default async function MonthlyReportPage({ searchParams }: { searchParams: Promise<{ m?: string }> }) {
  ...
  try {
    report = await getMonthlyReport(month);
  } catch {
    ...
    <Shell>
      <div className={s.doc}>
        <form method="GET" className={s.bar}>
          <label className="text-sm font-medium text-gray-700" htmlFor="m">Report for</label>
          <input
