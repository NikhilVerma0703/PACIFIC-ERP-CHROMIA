...
interface Piece {
  ...
  requirement: { pieceLabel: string | null; rowLetter: string | null; po: { poNumber: string } | null; length: number | null; width: number | null; dimUnit: string | null; sinkModel: string | null; sinkCuts: number | null } | null;
...
function PieceRow({ p, startMs, onStart, onComplete, completing, onRejected, onError }: {
  ...
  return (
    <tr className={`hover:bg-gray-50 transition ${activityRowClass(p.recent, isStarted, "bg-orange-50")}`}>
      ...
      <td className="px-5 py-3 text-xs text-gray-500">
        {p.requirement?.sinkModel ? <span className="font-medium text-orange-700">{p.requirement.sinkModel}</span> : "—"}
        {p.requirement?.sinkCuts ? <span className="ml-1 text-gray-400">({p.requirement.sinkCuts} cuts)</span> : null}
      </td>
      <td className="px-5 py-3 text-right">
        {!isStarted ? (
          <div className="flex items-center justify-end gap-1">
            <button onClick={onStart}
              className="bg-orange-100 hover:bg-orange-200 text-orange-700 px-3 py-1.5 rounded-lg text-xs font-semibold transition">
              Start
            </button>
            <RejectPieceButton pieceId={p.id} processType="SINK_CUTTING" onDone={onRejected} onError={onError} />
          </div>
        ) : (
          <div className="flex items-center justify-end gap-2">
            <span className="text-xs font-mono text-orange-600 animate-pulse">{elapsed}</span>
            <button onClick={onComplete} disabled={completing}
              className="bg-orange-600 hover:bg-orange-700 disabled:opacity-50 text-white px-3 py-1.5 rounded-lg text-xs font-semibold transition">
              {completing ? "…" : "Complete"}
            </button>
            <RejectPieceButton pieceId={p.id} processType="SINK_CUTTING" onDone={onRejected} onError={onError} />
          </div>
...
export default function FabSinkCuttingPage() {
  return (
    <ProcessSessionGate type="SINK_CUTTING">
      <SinkCuttingQueue />
    </ProcessSessionGate>
  );
}

function SinkCuttingQueue() {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Sink Cutting Queue</h1>
        ...
        <div className="flex items-center gap-3">
          {tab === "done" && (
            <input type="date" value={doneDate} onChange={e => { setDoneDate(e.target.value); loadDone(e.target.value); }}
              className="text-sm border border-gray-200 rounded-lg px-3 py-1.5 text-gray-700 bg-white focus:outline-none focus:ring-2 focus:ring-orange-200" />
          )}
          <div className="flex gap-1 bg-gray-100 rounded-lg p-1">
            <button onClick={() => setTab("open")}
              ...
              Open <span className="ml-1.5 text-xs bg-orange-100 text-orange-700 px-1.5 py-0.5 rounded-full">{pieces.length}</span>
            </button>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-xs text-gray-500">
                <tr>
                  ...
                  <th className="text-left px-5 py-3">Sink</th>
