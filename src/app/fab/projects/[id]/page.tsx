  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        <Link href="/fab/projects" className="text-gray-400 hover:text-gray-600 text-sm">← Projects</Link>
                  ...
                  return (
                    <tr key={req.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-5 py-2 text-gray-500">
                        {req.length && req.width ? `${req.length} × ${req.width}` : "—"}
                      </td>
                      <td className="px-5 py-2 text-center">{req.quantity}</td>
                      <td className="px-5 py-2 text-center">
                        {req.sinkRequired ? <span className="text-orange-600 font-medium">✓</span> : <span className="text-gray-300">—</span>}
                      </td>
                      <td className="px-5 py-2 text-center">
                        {req.polishRequired ? <span className="text-blue-600 font-medium">✓</span> : <span className="text-gray-300">—</span>}
                      </td>
                      <td className="px-5 py-2 text-gray-500">{slab?.slabCode ?? <span className="text-red-400">Not assigned</span>}</td>
                      <td className="px-5 py-2 text-center">
                        <span className={`text-xs px-2 py-0.5 rounded-full ${req.status === "ALLOCATED" ? "bg-green-100 text-green-700" : req.status === "RELEASED" ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-500"}`}>
                          {req.status}
                        </span>
                      </td>
                    </tr>
