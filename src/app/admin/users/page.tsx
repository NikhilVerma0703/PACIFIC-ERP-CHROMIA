...
import { canManageUsers, currentRole, currentUser, creatableRoles, assignableBranches, rankOf, ROLE_RANK, STATIONS, type RoleName } from "@/lib/rbac";
...
export default async function UsersPage() {
  ...
  const assignable = assignableBranches(role, myBranch);
  ...
  // The pairs that may be handed out as a SECOND role — the other job of
  // somebody who does two. Built from the SAME creatableRoles/assignableBranches
  // the Create form is filtered by, so a role nobody may create is not a role
  // anybody may be given as a second one, and flattened into one option per pair
  // because a role and a department are granted together or not at all
  // (lib/roleContext.ts treats a half-filled grant as no grant). The server
  // action rebuilds this identical list and matches the posted key against it —
  // the list below is a convenience for the admin, never the control.
  // International Sales is excluded, matching the server action exactly — those
  // logins have their own duty model and are refused a second job in BOTH
  // directions. Offering the option here and refusing it on submit would only
  // be a worse way to say the same no.
  const altGrantable = assignable
    .filter((b) => b !== "INTERNATIONAL_SALES")
    .flatMap((b) =>
      creatableRoles(role, b).map((r) => ({
        key: contextKey(r, b),
        label: `${roleLabelFor(r, b)} · ${BRANCH_LABEL[b] ?? b}`,
      })),
    );
  ...
  return (
    <Shell>
      ...
      <UserAdmin users={rows} creatable={creatableUnion} creatableByBranch={creatableByBranch} branches={assignable} stations={STATIONS} office={myBranch === "OFFICE"} showGlobal={role === "ADMIN"} myRole={role} myId={String(((await currentUser()) as any)?.id ?? "")} altGrantable={altGrantable} />
    </Shell>
  );
}
