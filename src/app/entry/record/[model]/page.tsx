...
export default async function SmartRecord({ params }: { params: Promise<{ model: string }> }) {
  ...
  // storage tanks (resin-prep source picker) ride in the same wave as the other
  // form data: the DISTINCT used to be awaited unconditionally BEFORE params were
  // even read, serializing one extra round-trip ahead of every entry form load
  // (measured 2026-08-14). Same query, same values, same optional-failure handling.
  const [options, silos, rmBags, storageTanks] = await Promise.all([
    selectOptions(model),
    ...
    (async () => {
      try {
        ...
        return rows.map((r) => String(r.tank_no));
      } catch { return [] as string[]; /* optional */ }
    })(),
  ]);
