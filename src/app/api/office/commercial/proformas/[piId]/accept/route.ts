...
// The order is not moved: acceptance is evidence, not a stage (OPEN-QUESTIONS
// §1/§2 — nothing gates on payment or acknowledgement yet).
import { commercialGate } from "@/lib/commercial/access";
import { json, deny, fail, handle, readBody, plain, str } from "@/lib/commercial/http";
import { logOrderEvent } from "@/lib/commercial/events";
import { canAccept, piLabel } from "@/lib/commercial/proforma-rules";
...
export async function POST(req: Request, { params }: Ctx) {
  ...
  return handle(async () => {
    ...
    if (!canAccept(pi.status)) fail(400, `Only an issued PI can be accepted (this revision is ${pi.status.toLowerCase()})`);
    ...
    await logOrderEvent(pi.orderId, "pi_accepted", {
      note: note ?? `Proforma ${piLabel(pi.number, pi.revision)} accepted by the customer`,
      by: g.user,
      payload: { proformaId: piId, number: pi.number, revision: pi.revision, total: snapshot.totalAmount, currency: snapshot.currency },
    });
