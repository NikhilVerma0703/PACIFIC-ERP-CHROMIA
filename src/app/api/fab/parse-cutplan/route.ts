...
//
// OCR of a printed cut plan (PDF/image) into slab-and-label groups.
//
// Already dead before this pass: nothing in the app called it, and the embedded
// Python pinned tesseract to a Windows install path
// (C:\Program Files\Tesseract-OCR\tesseract.exe), so it could not run on Vercel at all.
//
// Slab assignment now happens on the supervisor slab screen at /fab/supervisor/slabs,
// from the PO piece table the manager imported -- nothing is read back off paper.
//
