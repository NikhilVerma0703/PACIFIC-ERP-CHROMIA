...
//
// The export half of the CLO Optimizer round-trip: project requirements out as two
// CLO import workbooks (2cm and 3cm), to be nested outside the ERP and brought back
// through POST /api/fab/apply-slab-excel.
//
// Replaced mid-2026 by the supervisor slab screen at /fab/supervisor/slabs, which
// allocates pieces to slabs inside the app -- pick a slab, add its pieces, mark the
// sinks, send it to cutting -- so nothing has to leave and come back.
//
...
const GONE =
  "This endpoint has been retired. The CLO Optimizer round-trip was replaced by the supervisor slab screen at /fab/supervisor/slabs, which assigns pieces to slabs in the app.";
