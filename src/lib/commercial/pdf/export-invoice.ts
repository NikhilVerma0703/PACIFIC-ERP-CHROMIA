...
//   └────────────────────────────────────────────────────────────────────────┘
//   Marks & Nos │ Packages │ Description │ Thick │ No. of Slabs │ Quantity │ Unit │ Rate │ Amount
...
import { fmtWestern, fmtRateWestern, fmtDMY } from "@/lib/commercial/invoice-rules";
...
function itemsTable(s: InvoiceSnapshot): any {
  const body: any[][] = [[
    head("Marks &\nNos"),
    head("No. &\nKind of Pkgs"),
  ...
  s.lines.forEach((l, i) => {
    body.push([
      ...
      cell([l.description, l.design && l.design.toUpperCase() !== l.description.toUpperCase() ? l.design : ""].filter(Boolean).join("\n")),
  ...
  body.push([
    { ...cell("TOTAL", { bold: true, alignment: "right" }), colSpan: 4 }, {}, {}, {},
    cell(String(totalSlabs), { bold: true, alignment: "center" }),
    cell("", {}), cell("", {}), cell("", {}),
    cell(fmtWestern(s.subtotal, 3), { bold: true, alignment: "right" }),
  ]);
  return { table: { headerRows: 1, widths: [40, 52, "*", 32, 32, 50, 32, 42, 58], body }, margin: [0, 6, 0, 0] };
}

export function exportInvoiceDocDef(s: InvoiceSnapshot): any {
  const c = s.company;
  const b = s.bank;
          ...
          body: [
            [
              ...
              ], { boldFirst: true }),
              {
                stack: [
                  { columns: [
                    { stack: [{ text: "Invoice No.", fontSize: FS - 0.5, bold: true, color: "#444" }, { text: t(s.number), fontSize: FS, bold: true }] },
                    { stack: [{ text: "Date", fontSize: FS - 0.5, bold: true, color: "#444" }, { text: fmtDMY(s.date), fontSize: FS }] },
                  ] },
...
export async function generateExportInvoicePdf(snapshot: InvoiceSnapshot): Promise<Buffer> {
