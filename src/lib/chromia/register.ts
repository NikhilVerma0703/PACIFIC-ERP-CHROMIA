// Parser for the monthly production register — "PRO MAY" and its siblings, the
// workbook that has been the Chromia line's whole system of record.
//
// Ported from the module's lib/import/pro-register.ts. Pure: no database, no
// file I/O, so the mapping rules can be tested against real rows from the
// sheet. The explicit .ts on the import below is what lets `node --test` load
// this, the same reason extract.ts writes "./gstin.ts".
//
// Columns are addressed by INDEX, not by header name. The workbook has three
// merged header rows, and merged headers cannot be matched by name reliably.

import type { Disposition } from "./process.ts";
...
export function toDate(cell: Cell): Date | null {
 ...
 *
 * Real values from the register: "STOCK", "RECALIBRATE - HALF PRINT",
...
export function dispositionFromRemark(
  ...
  if (/RECALIB/.test(value)) return "RECALIBRATION";
  if (/SAMPLE/.test(value)) return "SAMPLE_CUTTING";
  if (/WASTE|SCRAP/.test(value)) return "WASTE";
  if (/STOCK/.test(value)) return "STOCK";
  if (hasDispatchDate) return "DISPATCH";
  return null;
}
...
/**
 * The state an imported row implies.
 *
 * A recalibration row is the one that needs care: sent with no return date
 * means the slab is still out — which is exactly the population the tracking
 * screen exists to surface, and importing it as anything else would hide the
 * backlog the module was built to find.
 */
export function statusForImportedRow(
  row: Pick<ParsedSlabRow, "disposition" | "recalReceivedDate">,
): string {
  switch (row.disposition) {
    case "DISPATCH": return "DISPATCHED";
    case "STOCK": return "IN_STOCK";
    case "SAMPLE_CUTTING": return "SAMPLE_CUT";
    case "WASTE": return "WASTE";
    case "RECALIBRATION":
      return row.recalReceivedDate ? "RECEIVED_FROM_RECALIBRATION" : "OUT_FOR_RECALIBRATION";
