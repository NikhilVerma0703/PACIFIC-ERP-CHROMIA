...
/**
 * The fabrication staff who may ADD sample stock — branch FABRICATION plus
 * rank >= OPERATOR, which is exactly what fabGate("EMPLOYEE") admits.
 *
 * ─────────────────── THIS USED TO STOP AT INCHARGE ──────────────────────────
 * It read `>= ROLE_RANK.INCHARGE`, on the reasoning: "a fab OPERATOR is not
 * included — the machine operator does not decide what is worth keeping."
 *
 * The owner overruled it, and he is right about his own floor: "I need space in
 * cutter to add some extra pieces as well, need to select from which slab, so
 * it will be added in the sample inventory as well."
 *
 * THE MAN AT THE SAW IS THE ONLY ONE WHO KNOWS THE OFFCUT EXISTS. He is holding
 * the piece and the supervisor is not in the room. Under the old rule a usable
 * remnant either went in the bin or waited for somebody to walk over — which is
 * how stone that had already been cut stopped reaching the shelf at all.
 *
 * ADD ONLY, AND THAT HAS NOT MOVED. The fabrication side still cannot see the
 * inventory, release, dispatch or deliver: SAMPLING_ACTORS above lists
 ...
 * "the fabrication side" rather than a rank.
 *
...
export function isFabStockContributor(user: unknown): boolean {
  ...
  return rankOf(String(u.role ?? "")) >= ROLE_RANK.OPERATOR;
}
