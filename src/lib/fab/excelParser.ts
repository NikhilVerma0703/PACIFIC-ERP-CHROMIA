...
//
// The Drawing Summary workbook parser -- one row per piece, carrying a drawing
// number, an area, a sink model and so on. Its only importer was
// POST /api/fab/parse-excel, retired in the same pass.
//
// Replaced mid-2026 by the flat piece-list intake: flatSheetParser.ts (the rules)
// and flatSheetWorkbook.ts (the .xlsx reader), plus poParser.parsePoPieceTable() for
// the PO PDF. Note flatSheetWorkbook.ts does NOT import anything from here -- it was
// written against flatSheetParser.ts from the start -- so this file goes whole.
//
