...
export function PageSkeleton({ rows = 5 }: { rows?: number }) {
  return (
    <div className="flex min-h-screen">
      {/* sidebar ghost (desktop). pacific-rail + overflow-hidden: collapses with
          the real rail when the sidebar is hidden (globals.css), so the skeleton
          does not paint a 256px rail the page is about to take away. */}
      <aside className="pacific-rail sticky top-0 hidden h-screen w-64 shrink-0 overflow-hidden border-r border-gray-200/70 bg-white/70 supports-[height:100dvh]:h-dvh md:flex">
        {/* padding on an inner fixed-width column, as in CollapsibleSidebar, so
            width: 0 on the aside really is 0 */}
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        {/* ghost of Shell's mobile top bar (61px): without it a phone painted the
            content 61px higher and shifted it down when the real header arrived */}
